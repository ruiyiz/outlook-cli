...
import type { FlaggedData } from "../hooks/use-flagged.ts";
...
export function FlaggedView({ flaggedThreads, loading, error, viewportHeight, isActive }: Pick<FlaggedData, "flaggedThreads" | "loading" | "error"> & { viewportHeight: number; isActive: boolean }) {
  ...
  const rows = buildRows(flaggedThreads, expandedThreads, loadedConversations, loadingConversations);
  ...
  if (loading && flaggedThreads.length === 0) {
    return (
      <Box paddingX={1} paddingY={1}>
  ...
  if (flaggedThreads.length === 0) {
    return (
      <Box paddingX={1} paddingY={1}>
        <Text dimColor>No flagged messages.</Text>
      </Box>
    );
  }
